...
This branch also contains an experimental native Win64 static port that does not require Cygwin or MSYS runtime DLLs. OpenSSL authentication is supported through a pinned, statically linked OpenSSL submodule, native Windows daemon mode (`-D`) is implemented, and `-Z` zerocopy uses Winsock `TransmitFile`. See `WINDOWS_STATIC_PORT.md` and `build-windows-static.ps1`.
